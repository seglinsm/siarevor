  ...
  const handleCheckout = async () => {
    try {
      ...
      if (checkoutUrl) {
        ...
        toast.error("Neizdevās izveidot pasūtījumu", {
          description: "Lūdzu mēģiniet vēlreiz.",
        });
      }
    } catch (error) {
      console.error('Checkout failed:', error);
      toast.error("Kļūda izveidojot pasūtījumu");
    }
  };
    ...
    <Sheet open={isOpen} onOpenChange={setOpen}>
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full bg-card">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle className="font-display text-xl">Grozs</SheetTitle>
          <SheetDescription>
            {totalItems === 0 ? "Jūsu grozs ir tukšs" : `${totalItems} prece${totalItems !== 1 ? 's' : ''} grozā`}
          </SheetDescription>
        </SheetHeader>
          ...
          {items.length === 0 ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Jūsu grozs ir tukšs</p>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t border-border bg-card">
                <div className="flex justify-between items-center">
                  <span className="text-lg font-semibold">Kopā</span>
